...
type User struct {
	...
	MemberLevel      interface{} `json:"member_level"`
	BposLevel        interface{} `json:"bpos_level"`
...
通过用户ID和Token获取用户信息
*/
func GetUserInfoBaseByToken(this *rgrequest.Client, id int, token string) (user User, err error) {
	host := rgconfig.GetStr(configCCHost)
	if host == "" {
		return user, errors.New("未配置" + configCCHost)
	}
	url := host + fmt.Sprintf("/internal/member/baseByToken?id=%d&token=%s", id, token)
